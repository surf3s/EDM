...
class e5_MainScreen(Screen):
    ...
    def warnings_and_errors_popup(self, warnings, errors, auto_dismiss=False):
        message_txt = '\n'
        message_txt += ',\n\n'.join(['Warning: ' + warning for warning in warnings])
        message_txt += ',\n\n'.join(['Error: ' + error for error in errors])
        return e5_MessageBox('Warnings and errors', message_txt, colors=self.colors, auto_dismiss=auto_dismiss)
    ...
    def show_delete_all_records(self, table_name=None):
        if not self.cfg.filename or not self.data.filename:
            self.popup = e5_MessageBox(APP_NAME, '\nOpen a CFG before trying to edit records.', call_back=self.dismiss_popup, colors=self.colors)
            self.popup.open()
            self.popup_open = True
            return
        if len(self.data.db.table(self.data.table if table_name is None else table_name).all()) == 0:
            self.popup = e5_MessageBox(APP_NAME, '\nThere are no data records to delete.', call_back=self.dismiss_popup, colors=self.colors)
            self.popup.open()
            self.popup_open = True
            return

        if not table_name:
            message_text = f'\n You are asking to delete all {len(self.data.db.table(self.data.table).all())} records in the current database table. Are you sure you want to do this?'
            self.delete_table = self.data.table
        else:
            message_text = f'\n You are asking to delete {len(self.data.db.table(self.data.table if table_name is None else table_name).all())} records from the {table_name} table. Are you sure you want to do this?'
    ...
    def save_geojson(self, path):
        ...
        self.popup = Popup(title="Export geoJSON file",
                            content=DataGridLabelAndProgressBar(col='Progress...', popup=True,
                                                                  ...
                                                                  size_hint=(.6, .15), auto_dismiss=False)
        self.popup.open()
        self.popup_open = True
...
class e5_SettingsScreen(Screen):
    ...
    def go_back(self, instance):
        self.popup = e5_MessageBox(APP_NAME, '\nNote that some changes to colors and size will not take effect until the program is restarted.',
                                    call_back=self.dismiss_popup,
                                    colors=self.colors, auto_dismiss=False)
        self.popup.open()
        self.popup_open = True
    ...
    def filter(self, instance):
        if instance.text == 'Clear Filter':
            ...
            self.popup = db_filter(default_field=self.filter_field, fields=['doc_id', 'Unit-ID'] + self.e5_cfg.fields(),
                                    colors=self.colors, call_back=self.apply_filter, auto_dismiss=False)
            self.popup.open()
            ...
            if len(self.data.db.table(self.data.table)) > 1:
                ...
                if all(field in last_record.keys() for field in ['X', 'Y', 'Z']):
                    ...
                    if dup:
                        ...
                        self.popup = e5_MessageBox(title='Warning', message=message, colors=self.colors, auto_dismiss=False)
                        self.popup.open()
        ...
        if instance.focus and not self.loading:
            ...
            if cfg_field:
                ...
                if cfg_field.inputtype in ['MENU', 'BOOLEAN']:
                    ...
                    self.popup = DataGridMenuList(title=cfg_field.prompt if cfg_field.prompt else instance.id, menu_list=cfg_field.menu,
                                                    menu_selected=instance.text, call_back=self.menu_selection,
...
class DataGridMenuList(Popup):
    ...
    def __init__(self, title, menu_list, menu_selected='', call_back=None, dismiss_call_back=None, colors=None, text_length=0, **kwargs):
        ...
        pop_content.add_widget(e5_button('Back', selected=True,
                                                 call_back=self.dismiss if dismiss_call_back is None else dismiss_call_back,
                                                 button_height=button_height,
                                                 colors=colors))
...
class DataGridTextBox(Popup):
    ...
    def __init__(self, title, label=None, text='', multiline=False, call_back=None, dismiss_call_back=None,
        ...
        buttons = e5_side_by_side_buttons(button_text,
                                            ...
                                            id=[title, 'add_button'],
                                            selected=[True, True],
                                            call_back=[self.dismiss if dismiss_call_back is None else dismiss_call_back, call_back],
                                            colors=self.colors)
...
class DataGridTableData(RecycleView):
    ...
    def editcell(self, key, field, db):
        ...
        if self.inputtype in ['MENU', 'BOOLEAN']:
            self.popup = DataGridMenuList(title=field, menu_list=cfg_field.menu,
                                            menu_selected=editcell_widget.text, call_back=self.menu_selection,
                                            colors=self.colors, text_length=text_length)
            self.popup.auto_dismiss = False
            self.popup.open()
...
class DataGridCasePanel(BoxLayout):
    ...
    def populate(self, data, fields, colors=None, call_back=None, revert=None):
        if data is not None and fields is not None:
            ...
            if not self.bottom_buttons:
                self.bottom_buttons = e5_side_by_side_buttons(text=['Revert', 'Save'],
                                                                call_back=[revert, call_back],
                                                                colors=self.colors,
                                                                button_height=self.calc_button_height())
                self.add_widget(self.bottom_buttons)
            self.bottom_buttons.opacity = 1
            self.bottom_buttons.disabled = False
        self.changed = False
    ...
    def depopulate(self):
        ...
        self.edit_list.clear_widgets()
        helptext = e5_label_wrapped('To edit a case here, first select a case in the data grid (first tab above) by clicking in the doc_id column.  The record will be highlighted.  Then it can be edited here.', colors=self.colors)
        self.edit_list.add_widget(helptext)
        self.changed = False
...
class DataGridDeletePanel(GridLayout):
    ...
    def populate(self, message=None, call_back=None, colors=None):
        ...
        if message is not None:
            ...
            self.add_widget(e5_label_wrapped('\nTo delete a case here, first select a case in the data grid view (first tab above) by clicking in the doc_id column.  The record wil be highlighted.  Then it can be deleted here.',
                                                popup=False, colors=self.colors, padding_x=10, padding_y=0))
...
class DataGridWidget(TabbedPanel):
                ...
                if data_record:
                    ...
                    for field in self.fields.fields():
                        if field in data_record.keys():
                            serialize_record += f" {field} : {data_record[field]}\n"
    ...
    def show_menu(self, instance, value):
        if instance.focus:
            ...
            if cfg_field:
                self.popup_field_widget = instance
                if cfg_field.inputtype in ['MENU', 'BOOLEAN']:
                    self.popup = DataGridMenuList(title=instance.id, menu_list=cfg_field.menu,
                                                    menu_selected=instance.text, call_back=self.menu_selection, colors=self.colors)
                    self.popup.auto_dismiss = False
                    self.popup.open()
                    self.popup_scrollmenu = self.get_widget_by_id(self.popup, 'menu_scroll')
                    self.popup_textbox = self.get_widget_by_id(self.popup, 'new_item')
    ...
    def addnew_record(self, instance):
        ...
        valid_data = self.cfg.validate_datarecord(new_record, self.data, new_record=True)
        if valid_data is True:
            unique_error = self.check_unique_together(None, new_record)
            if unique_error == '':
            ...
            self.open_popup(e5_MessageBox("Save error", valid_data, call_back=self.close_popup, colors=self.colors))
        ...
        if datatable is not None:
            ...
            if valid_data is True:
                if datatable.datagrid_doc_id is not None:
                    ...
                    self.open_popup(e5_MessageBox("Save error", "Internal error.  Not sure which record to overwrite.", call_back=self.close_popup, colors=self.colors))
            else:
                self.open_popup(e5_MessageBox("Save error", valid_data, call_back=self.close_popup, colors=self.colors))
    ...
    def check_unique_together(self, current_doc_id=None, new_record=''):
        unique_error = ''
        if self.cfg.unique_together and len(self.data) > 1:
            unique_key = self.get_unique_key(new_record)
            for record in self.data.all():
                if record.doc_id != current_doc_id or current_doc_id is None:
