...
__version__ = '1.0.16'
__date__ = 'June, 2022'
from edm.lib.constants import __program__
...
from edm.lib.blockdata import blockdata
from edm.lib.dbs import dbs
from edm.lib.e5_widgets import e5_label, e5_button, e5_MessageBox, e5_DatagridScreen, e5_RecordEditScreen, e5_side_by_side_buttons, e5_InfoScreen
from edm.lib.e5_widgets import edm_manual, DataGridTextBox, width_calculator, e5_SaveDialog, e5_LoadDialog, e5_PopUpMenu, e5_MainScreen
from edm.lib.e5_widgets import e5_LogScreen, e5_CFGScreen, e5_INIScreen, e5_SettingsScreen, e5_scrollview_menu, DataGridMenuList, SpinnerOptions
from edm.lib.e5_widgets import DataGridLabelAndField
from edm.lib.colorscheme import ColorScheme
from edm.lib.misc import restore_window_size_position, filename_only, platform_name
...
class DB(dbs):
    ...
    def unit_ids(self):
        name_list = [row['UNIT'] + '-' + row['ID'] for row in self.db.table(self.table)] if self.db else []
        return(name_list)

    def names(self, table_name):
        name_list = [row['NAME'] for row in self.db.table(table_name) if 'NAME' in row] if self.db else []
        return(name_list)
...
class CFG(blockdata):
    ...
    def build_default(self):
        ...
        # TODO should add hangle, vangle, sloped
